...
// dependency on FileSaver.js
// function saveText(text, filename, mimetype="application/dxf;charset=utf-8")
// {
//    var blob = new Blob(text, {type: mimetype});
//    saveAs(blob, filename); // FileSaver.js function
// }

// use "application/octet-stream" if you dont want the file to open in a new tab...
function saveText(text, filename, mimetype = "application/dxf;charset=utf-8") {
   // Ensure text is an array of strings or join it
...
// not safe in modern chrome (blocked), due to popup blocking against phishing
...
// trigger a textarea overlay to display the data with copy button
function showText(text, mimetype = "text/plain") {
   ...
   // --- Create a centered box for the text ---
   ...
   // --- Button container (for close + copy) ---
   ...
   copyBtn.textContent = "Copy"; // simple text
...
{
   console.log( "convertCanvasToImage: " + type );
   var img;
   if (type=="png")
...
var Canvas2Image = (function() {
   ...
   // }
   // trigger a download using an <a> element instead
   function saveFile(dataUrl, filename = "canvas.png") {
      console.log( "saveFile", link )
   ...
   // generates a full-screen <img> popup overlay
   var makeImageObject = function(strSource) {
      ...
      oImgElement.style.maxWidth = "90vw";
      oImgElement.style.maxHeight = "90vh";
      oImgElement.style.boxShadow = "0 0 20px rgba(0,0,0,0.8)";
      oImgElement.style.borderRadius = "10px";
      ...
      // Append everything
      overlay.appendChild(oImgElement);
      document.body.appendChild(overlay);
   };
   ...
   return {
      ...
      saveAsPNG : function(oCanvas, bReturnImg, iWidth, iHeight) {
         console.log("saveAsPNG", bReturnImg);
         if (!bHasDataURL) {
            return false;
         }
      ...
      saveAsBMP : function(oCanvas, bReturnImg, iWidth, iHeight) {
         ...
         if (bReturnImg) {
            makeImageObject(makeDataURI(strImgData, "image/bmp"));
         } else {
